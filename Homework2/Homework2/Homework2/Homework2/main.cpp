...
void AStar(int start_vertex, int end_vertex) {
	//vector<double> dist;
	double *dist = new double[Coordinates.size()];
	...
	while (open.size() > 0) {
		...
		if (current_node == end_vertex) {
			//			cout << "Started at node : " << start_vertex
			//					<< " and reached target node: " << current_node
			//					<< ". Input end node :" << end_vertex << endl;
			return;
		}
		...
		for (int i = 0; i < GraphWithWeights[current_node].size(); i++) {
			...
			if (in_closed.find(GraphWithWeights[current_node][i].second) != in_closed.end())
				continue;
			...
			if (length < dist[GraphWithWeights[current_node][i].first]) {
				...
				open.push(
					make_pair(
						length
						+ EuclideanDistance(
							Coordinates[current_node].first,
							Coordinates[current_node].second,
							Coordinates[end_vertex].first,
							Coordinates[end_vertex].second),
						GraphWithWeights[current_node][i].first));
			}
		}
...
void GenerateEuclidianHeuristics(double * heuristics, int end_vertex) {
	for (int i = 0; i < Coordinates.size(); i++) {
		heuristics[i] = EuclideanDistance(Coordinates[i].first,
			Coordinates[i].second, Coordinates[end_vertex].first,
			Coordinates[end_vertex].second);
	}
}
...
int main() {
	...
	cout << "Time (ms): " << duration / 25 << endl;
	cout << "Edges relaxed: " << average_edges_relaxed / 25 << endl;
	cout << "Closed size : " << in_closed.size()/25 << endl;
